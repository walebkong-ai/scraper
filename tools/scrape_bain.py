#!/usr/bin/env python3
"""
Bain & Company Google News Scraper
Scrapes Bain & Company news from Google News RSS feed as a reliable third-party source
...
# Search "Bain & Company" over the last 7 days (to ensure we catch weekends)
RSS_URL = "https://news.google.com/rss/search?q=%22Bain+%26+Company%22+when:7d&hl=en-US&gl=US&ceid=US:en"
SOURCE_NAME = "bain"
HOURS_FILTER = 48 # Filter down to recent business hours

def scrape_bain():
    """
    Scrape Google News RSS for Bain & Company Mentions over the last 48 hours.
    ...
    try:
        logging.info(f"Fetching Google News RSS for Bain from {RSS_URL}")
        ...
        for entry in feed.entries:
            try:
                ...
                if " - " in title:
                    ...
                    author = getattr(entry, 'source', {}).get('title', 'Google News (Bain Mention)')
        ...
        error_msg = f"Fatal error scraping Bain: {str(e)}"
        logging.error(error_msg)
        output["errors"].append(error_msg)
