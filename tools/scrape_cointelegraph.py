#!/usr/bin/env python3
"""
CoinTelegraph Crypto News Scraper
Scrapes crypto news from CoinTelegraph RSS feed
...
def scrape_cointelegraph():
    """
    Scrape CoinTelegraph RSS feed for articles from last 24 hours
    ...
    try:
        logging.info(f"Fetching CoinTelegraph RSS feed from {RSS_URL}")
        ...
        for entry in feed.entries:
            try:
                ...
                article = {
                    ...
                    "author": entry.get('author', 'CoinTelegraph'),
                    "is_saved": False
                }
        ...
        error_msg = f"Fatal error scraping CoinTelegraph: {str(e)}"
        logging.error(error_msg)
        output["errors"].append(error_msg)
